...
class BlurtSerializer(serializers.ModelSerializer):
	...
	class Meta:
		model = Blurt
		fields = ('id', 'content', 'username', 'display_name', 'email', 'profile_image', 'image', 'created_at')
...
class BlurtCommentSerializer(serializers.ModelSerializer):
	...
	profile_image = serializers.ImageField(source='author.profile_image', allow_null=True)

	class Meta:
		model = BlurtComment
		fields = ('id', 'blurt_id', 'author', 'display_name', 'content', 'profile_image', 'created_at')
